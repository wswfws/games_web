...
/** Симуляция хода: положить фигуру, сжечь линии, начислить очки (streak = 0) */
function simulate(
  board: Board,
  piece: TrayPiece,
  row: number,
  ...
  let gain = placed + cleared.clearedCells * t.clearCellPoints;
  if (lines > 1)
    gain += (lines - 1) * t.comboPerExtraLine + (lines >= t.megaComboMinLines ? t.megaComboBonus : 0);
  return { board: cleared.board, gain };
}
 ...
 */
function searchBest(board: Board, pieces: TrayPiece[], depth: number, t: AiTuning): number {
  ...
  for (const piece of pieces) {
    for (const p of allPlacements(board, piece.shape.cells)) {
      found = true;
      const sim = simulate(board, piece, p.row, p.col, t);
      const rest = pieces.filter((x) => x.uid !== piece.uid);
      const total = sim.gain + searchBest(sim.board, rest, depth - 1, t);
